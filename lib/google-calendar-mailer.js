...
function getAuthToken (apiCredentials) {
  ...
  rl.question('and enter the code from that page here: ', function (code) {
    ...
    apiCredentials.getToken(code, function (error, token) {
      if (error) {
        throw new Error(`Unable to generatate token: ${error}`)
      }
...
export class GoogleCalendarMailer {
  ...
  async runTask (task, mailer) {
    ...
    try {
      ...
      throw new Error(`[${task.name}] failed: ${e}`)
    }

    if (!events || !events.data || !Array.isArray(events.data.items)) {
      throw new Error(`[${task.name}] failed: Unexpected server response for event information: \n` + JSON.stringify(events))
    }

    if (typeof task.actions !== 'function') {
      throw new Error(`[${task.name}] failed: "action" is not a function`)
    }

    console.log(`[${task.name}]: Received (${events.data.items.length} events`)

    let messages = task.action(events, task.name) || []
    if (!Array.isArray(messages)) {
      throw new Error(`[${task.name}] failed: "action" did not return an array of messages to send`)
    }

    return Promise.all(messages.map(message => {
      console.log(`[${task.name}]: Sending email '${message[0]}'`)
